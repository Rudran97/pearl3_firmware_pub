...
class DebugShell:
    def __init__(self, device_id, comm_port, speed, dtot, read_timeout):
        self.dbg = dbg.Debugger(device_id=device_id, comm_port=comm_port, speed=speed, read_timeout=read_timeout, test_mode=False)
    ...
    def cmd_help(self):
        print("""
            ...
            info|i reg|registers|r:csr:prog:state:clic:
                   breakpoints|b:io:state|all            - Show registers:Show CSR:Show program info:Show all registers:
                                                           Show clic:Show breakpoints:Show peripheral registers:Show all
                                                           registers and CSR and program info.
            ...
            delete|d 0:1                                 - delete breakpoint trig0 or trig1.
            x m <addr> <words>                           - Read specific memory section.
                                                           For 'm' type, base address and number of words to be fetched
                                                           must be provided.
              s|ram:r|rom <page>                         - Read SRAM or ROM section 1kB at a time.
    ...
    def cmd_info(self, args):
        if not args:
            print("Usage: info <all|reg|csr|prog|clic|breakpoints|io>")
            return
    ...
    def cmd_break(self, args):
        ...
        if args[0] == "0":
            if self.dbg.setTrigger0(addr_hex):
                self.bpts["Breakpoint 0"] = (addr_hex, True)
        elif args[0] == "1":
            if self.dbg.setTrigger1(addr_hex):
                self.bpts["Breakpoint 1"] = (addr_hex, True)
        else:
            print("Only trigger0 and trigger1 are supported")
            return
    ...
    def cmd_delete(self, args):
        ...
        if module == 0:
            if self.dbg.removeTrigger0():
                self.bpts["Breakpoint 0"] = (f"{0:08x}", False)
        elif module == 1:
            if self.dbg.removeTrigger1():
                self.bpts["Breakpoint 1"] = (f"{0:08x}", False)
        else:
            print("Only trigger0 and trigger1 are supported")
            return
    ...
    def cmd_mem(self, args):
        if not args:
            print("Usage: x mem:ram:rom:io:clic <addr> <words>:<page>:<page>::")
            return
    ...
    def cmd_enter_debug(self, args):
        ...
        self.dbg.enterDebug(tot_val=self.dtot, use_dtot=True, prg_halt=prg_halt)
        self.running = True
...
class DebugTUI:
    ...
    csr_list = {0: "misa", 1: "mvendorid", 2: "marchid", 3: "mimpid", 4: "mhartid", 5: "minstret", 6: "mstatus", 7: "mie", 8: "mtvec", 9: "mip", 10: "mcause", 11: "mepc", 12: "mscratch", 13: "mtval", 14: "tselect", 15: "tdata1", 16: "tdata2", 17: "dcsr", 18: "dpc", 19: "dscratch0", 20: "dscratch1"}
    ...
    def viewRegion(self, addr, value, type, header_name="Rom", cell_sep=3):
        cells = [self.format_cells(format(int(str(addr), 16) + i * 4, "08x"), "", value[i], type=type) for i in range(len(value))]
